    ...
    getStrictSchema() {
        return {
            ...
            properties: {
                ...
                gameTime: {
                    type: 'object',
                    description: '游戏内时间（非现实时间）。必须根据本回合剧情活动量合理推进时间。重要：剧情需要时间发展——请根据当前题材和具体活动内容自行判断时间消耗（不要套用固定模板）。通用参考：短暂行为15-30分钟，中等行为1-2小时，长时间行为数小时-半天，全天行为推进到傍晚/晚上，睡眠/过夜推进到次日(date+1)，多日事件推进数日(date+N)，长期时间跳跃推进数周-数月。绝对不能在几千字的丰富剧情后只推进几分钟。如果story提到"第二天""数天后""数月后"等，date/time必须相应跳转',
                    properties: {
